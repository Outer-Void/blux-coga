...
```bash
pkg install python3
./CogA_mux.sh run --input path/to/problem.json --output-dir out/
```
...
./CogA_proot.sh run --input path/to/problem.json --output-dir out/
```

### Linux (Debian/Ubuntu)

```bash
./CogA.sh run --input path/to/problem.json --output-dir out/
```
...
```powershell
.\CogA.ps1 run --input path\to\problem.json --output-dir out\
```
